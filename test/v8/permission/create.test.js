...
test('POST /v8/permission returns 401 when not logged in', async t => {
  const res = await request(app).post('/v8/permission/bulk')
    .send({})
...
test('POST /v8/permission returns 200', async t => {
  ...
  const res = await request(app).post(`/v8/permission/bulk?instance_id=${instance_id}`)
